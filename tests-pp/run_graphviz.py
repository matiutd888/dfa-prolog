...
def dfa_to_graphviz(dfa, out_file, out_png_file):
...
if __name__ == '__main__':
    ...
    for file_path in AUTOMATA_DIR.glob('*.txt'):
        ...
        for dfa_name, dfa in dfas.items():
            ...
            if dir is not None:
                (GRAPHVIZ_DIR / 'src' / dir).mkdir(exist_ok=True)
                (GRAPHVIZ_DIR / dir).mkdir(exist_ok=True)
                out_file = GRAPHVIZ_DIR / 'src' / dir / (name + '_' + dfa_name + '.dot')
                out_png_file = GRAPHVIZ_DIR / dir / (name + '_' + dfa_name + '.png')
            dfa_to_graphviz(dfa, out_file, out_png_file)
